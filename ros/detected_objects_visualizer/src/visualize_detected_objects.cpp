...
VisualizeDetectedObjects::ObjectsToBoxes(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  for (auto const &object: in_objects.objects)
  {
    ROS_INFO("to Boxes ____");
    // ROS_INFO("IS Object Valid %d", IsObjectValid(object));
    // ROS_INFO("Object Label %s", object.label);
    // ROS_INFO("ObjectsMaxSize %d", (object.dimensions.x + object.dimensions.y + object.dimensions.z));
